...
abstract class SHFactory
{
	...
	public static function getDispatcher($name = 'SHFactory')
	{
		if (!isset(self::$dispatcher[$name]))
		{
			...
			elseif (Version::MAJOR_VERSION < 4) {
				...
				// J4.0+ 
				self::$dispatcher[$name] = Factory::getApplication()->getDispatcher();
				//self::$dispatcher[$name] = new EventDispatcher;
	 ...
	 *
	 * @return  Registry  Registry of the configuration options.
	 *
	 * @since   2.0
	 */
		...
		if (!isset(self::$config))
		{
			if ($type === 'sql')
			{
				if (!isset($options['handler']))
				{
					...
					$options['handler'] = Factory::getDbo();
				}
	...
	public static function getUserAdapter($user, $type = null, $options = array())
	{
		...
		if (empty($username))
		{
			throw new RuntimeException(Text::_('LIB_SHFACTORY_ERR_2121'), 2121);
		}
		...
		if (!isset(self::$adapters[$username]))
		{
			...
			if ($blacklist = (array) json_decode($config->get('user.blacklist') ?? ''))
			{
				...
				{
					throw new RuntimeException(Text::sprintf('LIB_SHFACTORY_ERR_2125', $username), 2125);
				}
			}

			// If the JUser ID has been specified then use it (more efficient)
			if ($id = isset($credentials['id']) ? (int) $credentials['id'] : UserHelper::getUserId($username))
			{
			...
			{
				throw new RuntimeException(Text::sprintf('LIB_SHFACTORY_ERR_2123', $class), 2123);
			}
		}
		else
		{
	 ...
	 *
	 * @return  Registry  Registry of configuration.
	 *
	 * @since   2.0
	 */
	protected static function createDBConfig( $handler, $table)
	{
	...
	protected static function createFileConfig($file, $namespace)
	{
		...
		throw new RuntimeException(Text::sprintf('LIB_SHPLATFORM_ERR_1121', $file, $name), 1121);
	}
}
