...
abstract class SHLdapHelper
{
	 ...
	 */
	public static function getConfig($id = null, Registry $registry = null, $source = self::CONFIG_AUTO)
	{
		...
		if ($source === self::CONFIG_SQL)
		{
			...
			if (empty($id))
			{
				...
				{
					// No results found
					throw new InvalidArgumentException(Text::_('LIB_SHLDAPHELPER_ERR_10604'), 10604);
				}
			}
			...
			elseif (is_numeric($id))
			{
				...
				{
					// No result found
					throw new InvalidArgumentException(Text::sprintf('LIB_SHLDAPHELPER_ERR_10605', $id), 10605);
				}
			}
			...
			else
			{
				...
				{
					// No result found
					throw new InvalidArgumentException(Text::sprintf('LIB_SHLDAPHELPER_ERR_10606', $id), 10606);
				}
			}
		}
		elseif ($source === self::CONFIG_PLUGIN)
		{
			...
			{
				// Invalid plugin
				throw new InvalidArgumentException(Text::sprintf('LIB_SHLDAPHELPER_ERR_10603', $name), 10603);
			}
		}
		elseif ($source === self::CONFIG_FILE)
		{
			...
			else
			{
				if (is_numeric($id))
				{
					...
					else
					{
						...
						throw new InvalidArgumentException(Text::sprintf('LIB_SHLDAPHELPER_ERR_10609', $id), 10609);
					}
				}
				elseif (is_string($id))
				{
				...
				{
					// Invalid id argument
					throw new InvalidArgumentException(Text::_('LIB_SHLDAPHELPER_ERR_10610'), 10610);
				}
			}
			...
			else
			{
				...
				else
				{
					...
					throw new RuntimeException(Text::sprintf('LIB_SHLDAPHELPER_ERR_10608', $file), 10608);
				}
			}
		}
		else
		{
			// Invalid source
			throw new InvalidArgumentException(Text::_('LIB_SHLDAPHELPER_ERR_10601'), 10601);
		}
	...
	protected static function createFileConfig($namespace, $file = null)
	{
		...
		if (!class_exists($class))
		{
			if (is_null($file) || !file_exists($file))
			{
				...
				throw new InvalidArgumentException(Text::sprintf('LIB_SHLDAPHELPER_ERR_10622', $class, $file), 10622);
			}
		...
		// Failed to create config file
		throw new RuntimeException(Text::sprintf('LIB_SHLDAPHELPER_ERR_10621', $class), 10621);
	}
	...
	public static function convertConfig($parameters, $convert = 'SHLdap')
	{
		...
		if ($params->get('auth_method') && $params->get('search_string') || $params->get('users_dn'))
		{
			...
			$converted['ldap_userAccountControl']		= $params->get('ldap_userAccountControl');
	 ...
	 * via LDAP.
	 *
	 * @param   User|integer|array  $user  Optional user id (if null then uses current user).
	 *
	...
	public static function triggerEvent($event, $args = null)
	{
		if (Version::MAJOR_VERSION < 4) 
			...
			//$results = SHFactory::getDispatcher('ldap')->triggerEvent($event, $args); //fix for Joomla 4
			$results = Factory::getApplication()->triggerEvent($event, $args); 
		}
	...
	public static function commitChanges($adapter, $log = false, $throw = true)
	{
		...
		if ($log)
		{
			...
			{
				if ($commit->status === Log::INFO)
				{
					SHLog::add($commit->message, 10634, Log::INFO, 'ldap');
				}
				else
				{
					SHLog::add($commit->message, 10636, Log::ERROR, 'ldap');
					SHLog::add($commit->exception, 10637, Log::ERROR, 'ldap');
				}
			}
		}
		...
		if (!$results->status)
		{
			if ($throw)
			{
				throw new RuntimeException(Text::_('LIB_SHLDAPHELPER_ERR_10638'), 10638);
			}
			else
			{
				return $results;
