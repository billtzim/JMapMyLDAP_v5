 ...
 * @since       2.0
 */
class PlgAuthenticationSHAdapter extends CMSPlugin
{
	/**
	...
	public function onUserAuthenticate($credentials, $options, &$response)
	{
		...
		if (empty($credentials['password']))
		{
			...
			$response->error_message = Text::_('PLG_AUTHENTICATION_SHADAPTER_ERR_12602');
		...
		if (!$this->_checkPlatform())
		{
			...
			$response->status = Authentication::STATUS_FAILURE;
			$response->error_message = Text::_('PLG_AUTHENTICATION_SHADAPTER_ERR_12601');
		...
		else
		{
			...
			if ($domain = Factory::getApplication()->input->get('login_domain', null, 'cmd'))
			{
				$credentials['domain'] = $domain;
			}
		}
		...
		try
		{
			...
			throw new Exception(Text::_('JGLOBAL_AUTH_NO_USER'), 999);
		}
		catch (Exception $e)
		{
			...
			SHLog::add($e, 12621, Log::ERROR, 'auth');
	 ...
	 * @param   array  $options   Array of extra options
	 *
	 * @return  AuthenticationResponse  Authentication response object
	 *
	 * @since   2.0
	 */
	public function onUserAuthorisation($response, $options = array())
	{
		...
		$retResponse = new AuthenticationResponse;
		...
		try
		{
			...
			if (!is_array($attributes) || !count($attributes))
			{
				...
				throw new Exception(Text::_('PLG_AUTHENTICATION_SHADAPTER_ERR_12611'), 12611);
			}
		}
		catch (Exception $e)
		{
			...
			$response->status = Authentication::STATUS_FAILURE;
			$response->error_message = Text::_('PLG_AUTHENTICATION_SHADAPTER_ERR_12611');

			// Process a error log
			SHLog::add($e, 12622, Log::ERROR, 'auth');
		...
		if ($adapter->getEnabledDisabled() == 514 || $adapter->getEnabledDisabled() == 66050)
			$response->block = 1;
		else
			$response->block = 0;
		 ...
		 */
		SHLog::add(Text::sprintf('PLG_AUTHENTICATION_SHADAPTER_INFO_12612', $response->username), 12612, Log::INFO, 'auth');

		$retResponse->status = Authentication::STATUS_SUCCESS;
