 ...
 * @since       2.0
 */
class PlgLdapMapping extends CMSPlugin
{
	...
	public function onUserBeforeSave($user, $isNew, $new)
	{
		...
		{
			SHLog::add($e, 12022, Log::ERROR, 'ldap');
		}
	}
	...
	public function onUserAfterSave($user, $isNew, $success, $msg)
	{
		...
		{
			SHLog::add($e, 12023, Log::ERROR, 'ldap');
		}
	}
		...
		if (count($addedGroups) || count($removedGroups))
		{
			...
			if (count($groupsToAdd) || count($groupsToRemove))
			{
						...
						foreach ($groupsToAdd as $group)
						{
							try
							{
									...
									|| (array_search($memberDn, $users) === false))
								{
									...
									SHLog::add(Text::sprintf('PLG_LDAP_MAPPING_INFO_12038', $username, $group), 12038, Log::INFO, 'ldap');
								}
							}
							catch (Exception $e)
							{
								// An error occurred trying to add the group
								SHLog::add(Text::sprintf('PLG_LDAP_MAPPING_ERR_12034', $group, $username), 12034, Log::ERROR, 'ldap');
								SHLog::add($e, 12034, Log::ERROR, 'ldap');
							}
						}
					}
						...
						foreach ($groupsToRemove as $group)
						{
							try
							{
								...
								if ($key !== false)
								{
									...
									$ldap->replaceAttributes($group, array($attribute => $users));
									SHLog::add(Text::sprintf('PLG_LDAP_MAPPING_INFO_12039', $username, $group), 12039, Log::INFO, 'ldap');
								}
							}
							catch (Exception $e)
							{
								// An error occurred trying to remove the group
								SHLog::add(Text::sprintf('PLG_LDAP_MAPPING_ERR_12036', $group, $username), 12036, Log::ERROR, 'ldap');
								SHLog::add($e, 12036, Log::ERROR, 'ldap');
							}
						}
					}
				}
				else
				{
					...
					SHLog::add(Text::sprintf('PLG_LDAP_MAPPING_ERR_12031', $username), 12031, Log::ERROR, 'ldap');
				}
			}
		}
	}
	...
	{
		//error_log("onUserLogin 1");
		if ($this->params->get('onlogin'))
		{
			//error_log("onUserLogin 2");
	...
	public function onLdapSync(&$instance, $options = array())
	{
		...
		try
		{
			...
			if (!(is_array($userGroups) && count($userGroups)))
			{
				...
				SHLog::add(Text::sprintf('PLG_LDAP_MAPPING_DEBUG_12008', $username), 12008, Log::DEBUG, 'ldap');
			...
			{
				/*
				 * Find the groups that require adding then add them to the User
				...
				{
					SHLog::add(Text::sprintf('PLG_LDAP_MAPPING_DEBUG_12011', $group, $username), 12011, Log::DEBUG, 'ldap');
			...
			if ($this->removal !== self::NO)
			{
				...
				{
					SHLog::add(Text::sprintf('PLG_LDAP_MAPPING_DEBUG_12013', $group, $username), 12013, Log::DEBUG, 'ldap');
		...
		{
			SHLog::add($e, 12021, Log::ERROR, 'ldap');
	...
	protected function doSetup()
	{
		...
		if (is_null($done))
		{
			...
			$UserGroups = SHUserHelper::getJUserGroups();
			$UserGroupsKey = array_fill_keys($UserGroups, '');
			...
			if ($this->removal === self::YESDEFAULT)
			{
				...
				$this->managed = array_diff($UserGroups, $this->unmanaged);
			}
			...
			if (!count($this->entries))
			{
				...
				SHLog::add(Text::_('PLG_LDAP_MAPPING_DEBUG_12006'), 12006, Log::DEBUG, 'ldap');
				$done = false;
			}
		}
	 ...
	 *
	 * @param   User  $user     Specify user for adding groups.
	 ...
	 * @since   1.0
	 */
	public static function getGroupsToAdd(User $user, array $mapList)
	{
		$addGroups = array();
	 ...
	 * @since   1.0
	 */
	public static function getGroupsToRemove(User $user, array $mapList, array $managed)
	{
		$removeGroups = array();
