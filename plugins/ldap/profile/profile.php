 ...
 * @since       2.0
 */
class PlgLdapProfile extends CMSPlugin
{
	/**
	...
	/**
	 * Synchronise email with joomla database
	 * 0-No Sync | 1-Sync From LDAP
	 *
	 * @var    integer
	 * @since  2.1
	 */
	protected $sync_enabled_disabled = null;
	...
	public function __construct(&$subject, $config = array())
	{
		...
		$this->sync_enabled_disabled = (int) $this->params->get('sync_enabled_disabled', 1);
	 ...
	 *
	 * @param   User  &$instance  A User object for the authenticating user.
	...
	public function onUserLogin(&$instance, $options = array()) {
		if ($this->params->get('onlogin')) {
			...
			SHLog::add('cannot get onlogin param', 15078, Log::ERROR, 'ldap');
	}
	...
	public function onLdapSync(&$instance, $options = array())
	{
		...
		{
			// !!!!!
			//29/11/2023 there is a mixup array or object
			//if it runs in cron then it is an object and access individual properties using arrow ->
			//if it run in sso then it is an array and access  individual properties using brackets []
			// !!!!!
			...
			if (gettype($instance) != 'object') {
				...
				if ($this->use_profile) {
					...
					$result = $this->saveProfile($instance_id, $username, $adapter, $options);
		...
		{
			SHLog::add($e, 12231, Log::ERROR, 'ldap');
	 ...
	 * @param   object  $data     Associated data for the form (this should be User in this context)
	 *
	 * @return  boolean
	 *
	 * @since   2.0
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		elseif (!Factory::getUser()->guest)
		{
			 ...
			 */
			if ($inForm = Factory::getApplication()->input->get('jform', false, 'array'))
			{
				...
				if ($id === 0)
				{
					...
					if (count($results = SHFactory::getDispatcher('ldap')->triggerEvent('askUserCreation')))
					{
		...
		if ($showForm)
		{
			...
			//if (($xml = Factory::getXML($xmlPath, true)) && ($form->load($xml, false, false)))
			if (($xml = simplexml_load_file($xmlPath)) && ($form->load($xml, false, false)))
			{
	...
	public function onUserAfterDelete($user, $success, $msg)
	{
		...
		if ($success)
		{
			try
			{
				...
				SHLog::add(Text::sprintf('PLG_LDAP_PROFILE_INFO_12211', $user['username']), 12211, Log::INFO, 'ldap');
			}
			catch (Exception $e)
			{
				SHLog::add($e, 12234, Log::ERROR, 'ldap');
			}
		}
	}
	 ...
	 * @since   2.0
	 */
	public function onSHUserBeforeSave($user, $isNew, $new)
	{
		...
		{
			SHLog::add($e, 12232, Log::ERROR, 'ldap');
	...
	public function onUserAfterSave($user, $isNew, $success, $msg)
	{
		...
		if ($success)
		{
			...
			{
				SHLog::add($e, 12233, Log::ERROR, 'ldap');
			}
		}
	}
	...
	public function onLdapBeforeRead($adapter, $options = array())
	{
		if ($this->use_profile)
		{
			...
			{
				SHLog::add($e, 12235, Log::ERROR, 'ldap');
			}
		}
	}
	...
	protected function getXMLFileName($domain)
	{
		...
		if (empty($file))
		{
			throw new RuntimeException(Text::_('PLG_LDAP_PROFILE_ERR_12204'), 12204);
		}
		...
		if (!file_exists($xmlPath))
		{
			if (!$this->use_domain || ($file == $domain && $file == $this->profile_name))
			{
				...
				throw new RuntimeException(Text::sprintf('PLG_LDAP_PROFILE_ERR_12201', $file), 12201);
			}
			else
			{
	...
	protected function getXMLFields($domain = null)
	{
		...
		if ($xml = simplexml_load_file($xmlPath))
		{
			...
			{
				SHLog::add(Text::_('PLG_LDAP_PROFILE_DEBUG_12202'), 12202, Log::DEBUG, 'ldap');

				// Register the JForm rules classes
				if (file_exists($this->profile_base . '/rules'))
				{
					Form::addRulePath($this->profile_base . '/rules');
				}

				// Register the JForm fields classes
				if (file_exists($this->profile_base . '/fields'))
				{
					Form::addFieldPath($this->profile_base . '/fields');
				}
		...
		throw new RuntimeException(Text::sprintf('PLG_LDAP_PROFILE_ERR_12203', $xmlPath), 12203);
	}

	/**
	 * Initialise the synchronisation of the name and email fields from LDAP.
	 *
	 * @param   User          &$instance  Reference to the active joomla user.
	...
	protected function saveProfile($userId, $username, $adapter, $options = array())
	{
		...
		SHLog::add(Text::sprintf('PLG_LDAP_PROFILE_DEBUG_12221', $username), 12221, Log::DEBUG, 'ldap');
		...
		*/
		...
		if (count($results))
		{
			...
			SHLog::add(
				Text::sprintf(
					...
					$username,
					$return == 1 ? Text::_('PLG_LDAP_PROFILE_SUCCESS') : Text::_('PLG_LDAP_PROFILE_FAIL')
				), 12225, Log::DEBUG, 'ldap'
			);
			...
			SHLog::add(Text::sprintf('PLG_LDAP_PROFILE_INFO_12224', $username), 12224, Log::INFO, 'ldap');
		...
		else
		{
			...
			SHLog::add(
				Text::sprintf('PLG_LDAP_PROFILE_DEBUG_12226', $username), 12226, Log::DEBUG, 'ldap'
			);
	 ...
	 *
	 * @param   integer  $userId  The user ID of the User
	 *
	 * @return  boolean  True on success
	 *
	 * @since   2.0
